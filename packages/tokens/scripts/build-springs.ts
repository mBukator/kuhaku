 ...
 *
 * The spring parameters are read from motion.css, where they are declared once.
 * lib/motion in the registry mirrors the same values for motion/react.
 */
...
const SPRING_PARAMETERS = ["stiffness", "damping", "mass"] as const;

const CSS_DIRECTORY = join(dirname(fileURLToPath(import.meta.url)), "..", "src", "css");
const SPRING_DECLARATION =
    /--motion-spring-([a-z]+)-(stiffness|damping|mass):\s*([^;]+);/g;

const REST_THRESHOLD = 0.001; // settled within 0.1% of target
const TOLERANCE = 0.005; // max piecewise-linear error, 0.5%
const MAX_POINTS = 40;

function isSpringParameter(value: string): value is keyof Spring {
    return (SPRING_PARAMETERS as readonly string[]).includes(value);
}

/**
 * Collects every --motion-spring-{name}-{parameter} declaration in motion.css.
 * Parameters are declared once, in :root; a second declaration would make the
 * last one win silently, so it fails instead.
 */
function readSprings(): Record<string, Spring> {
    const css = readFileSync(join(CSS_DIRECTORY, "motion.css"), "utf8");
    const declared: Record<string, Partial<Spring>> = {};

    for (const [, name, parameter, rawValue] of css.matchAll(SPRING_DECLARATION)) {
        if (!isSpringParameter(parameter)) continue;

        const value = Number(rawValue.trim());
        if (!Number.isFinite(value) || value <= 0) {
            throw new Error(
                `motion.css: --motion-spring-${name}-${parameter} must be a positive number, got "${rawValue.trim()}"`
            );
        }
        if (declared[name]?.[parameter] !== undefined) {
            throw new Error(
                `motion.css: --motion-spring-${name}-${parameter} is declared more than once`
            );
        }

        declared[name] = { ...declared[name], [parameter]: value };
    }

    const springs: Record<string, Spring> = {};
    for (const [name, parameters] of Object.entries(declared)) {
        const { stiffness, damping, mass } = parameters;
        if (stiffness === undefined || damping === undefined || mass === undefined) {
            const missing = SPRING_PARAMETERS.filter(
                (parameter) => parameters[parameter] === undefined
            );
            throw new Error(
                `motion.css: --motion-spring-${name} is missing ${missing.join(", ")}`
            );
        }
        springs[name] = { stiffness, damping, mass };
    }

    if (Object.keys(springs).length === 0) {
        throw new Error("motion.css: no --motion-spring-* parameters found");
    }
    return springs;
}
...
const out = join(CSS_DIRECTORY, "springs.generated.css");
